...
labels = ['A','B']
ax.pie(ab_counts.first_name, labels=labels, autopct='%1.1f%%')

ax.axis('equal')
f.tight_layout()
f.savefig('ab_test_pie_chart.png')
f.show()
...
a,pval,c,d = chi2_contingency([[250,325],[2254,2175]])
print pval
...
just_app = df[pd.notnull(df.application_date)]
just_app.head()
...
member_counts = just_app.groupby(['ab_test_group','is_member']).first_name.count().reset_index()
...
from scipy.stats import chi2_contingency
a,pval,c,d = chi2_contingency([[200,250],[50,75]])
print pval
...
from scipy.stats import chi2_contingency
a,pval,c,d = chi2_contingency([[200,250],[2304,2250]])
print pval
...
lbls = ['Fitness Test', "No Fitness Test"]

app_fig = plt.figure()
ax1 = app_fig.add_subplot(111)
ax1.bar(range(len(app_pivot)),app_pivot['Percent with Application'].values*100)
ax1.set_xticks([0,1])
...
app_fig.savefig('percent_of_vis_who_apply.png')


# In[80]:


app_member_fig = plt.figure()
ax2 = app_member_fig.add_subplot(111)
ax2.bar(range(len(member_pivot)),member_pivot['Percent Purchase'].values*100)
ax2.set_xticks([0,1])
ax2.set_xticklabels(['Fitness Test', 'No Fitness Test'])
ax2.set_title('Percent of Applicants Who Purchase a Membership')
ax2.set_yticks(range(0,85,20))
ax2.set_yticklabels(['0%','20%','40%','60%','80%'])
app_member_fig.savefig('percent_of_apps_who_purchase_membership.png')


# In[82]:


final_member_fig = plt.figure()
ax3 = final_member_fig.add_subplot(111)
ax3.bar(range(len(final_member_pivot)),final_member_pivot['Percent Purchase'].values*100)
ax3.set_xticks([0,1])
...
final_member_fig.savefig('percent_of_vis_who_purchase_membership.png')
